        ...
        while True:
            ...
            try:
                if sim_command.lower() in ["f", "fin", "finish", "save and finish"]:
                    ...
                    coo_ordinate = [int(i.strip()) for i in sim_command[16:].split(",")]
                    for i in self.maze.access_tile(coo_ordinate)["events"]:
                        ret_str += f"{i}\n"
                    ...
                    # Ex: print tile event 50, 30
                    coo_ordinate = [int(i.strip()) for i in sim_command[18:].split(",")]
                    for key, val in self.maze.access_tile(coo_ordinate).items():
                        ret_str += f"{key}: {val}\n"
